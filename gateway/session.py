...
that requires raw IDs).  All third-party chat-messenger platforms were removed
from this fork, so this stays empty and PII redaction is always applied."""
...
def build_channel_continuity_note(
    ...
    Slack and Discord were removed from this fork, so no retained platform
    triggers this note; the function is kept as a no-op so the call site in
    ``gateway.run`` stays unchanged and always receives ``None``.
    """
    return None
...
def build_session_key(
    ...
    # (Messages arriving in an auto-created thread carry chat_type="thread",
    # so this only rewrites the initiating channel message's slot.)
...
class SessionStore:
    ...
    def _recovered_row_matches_source_scope(
        ...
        The workspace-scoped origin guard that this helper enforced was only
        needed for removed Slack group/channel rows; it is inert in this fork
        and always accepts recovered rows.
        """
        return True
