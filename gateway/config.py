...
# Platforms whose port-binding status depends on connection mode. All
# mode-conditional platforms were removed from this fork, so no platform
# is mode-conditional here; every retained port-binding platform always
# binds (#52563).
...
#
# All token-based third-party messengers were removed from this fork; the
# retained platforms (email, api_server, webhook, msgraph_webhook,
# wecom_callback, relay) authenticate via other means, so this stays empty.
...
class GatewayConfig:
    ...
    def _is_platform_connected(self, platform: Platform, config: PlatformConfig) -> bool:
        ...
        # Generic token/api_key auth covers the retained platforms.
...
def _validate_gateway_config(config: "GatewayConfig") -> None:
    ...
    if has_usable_secret is not None:
        for platform, pconfig in config.platforms.items():
            if not pconfig.enabled:
                continue
            env_name = _token_env_names.get(platform) or platform.value
            token = pconfig.token
