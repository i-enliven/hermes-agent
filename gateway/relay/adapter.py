...
class RelayAdapter(BasePlatformAdapter):
    ...
    def supports_status_text(self) -> bool:  # type: ignore[override]
        ...
        """
        # Slack removed from this fork — no text status line.
        return False
    ...
    def _stamp_slack_session_thread(self, event) -> None:
        ...
        """
        # Slack removed from this fork — no session-thread stamping.
        return None
    ...
    def _capture_scope(self, event) -> None:
        ...
        try:
            ...
            # platforms). source.platform is a Platform enum (e.g. Platform.EMAIL,
            # mapped from the connector's "email" by ws_transport _frame_to_event);
    ...
    def _discord_interaction_to_event(self, forward):
        """Convert a forwarded Discord interaction body to a MessageEvent.

        Discord platform removed from this fork — no interaction conversion.
        """
        return None
    ...
    def _resolve_reply_to_for_send(
        ...
        if reply_to is None:
            return None
        # Slack removed from this fork — pass-through reply_to.
        return reply_to
    ...
    def _apply_slack_thread_anchor(
        ...
        *,
        ...
        # Slack removed from this fork — no slack thread-anchor mirroring.
    ...
    def _with_status_thread_anchor(
        ...
        """
        # Slack removed from this fork — no slack status-thread anchoring.
    ...
    async def stop_typing(
        ...
        if self._transport is None:
            return
        # Slack removed from this fork — no-op.
        return
