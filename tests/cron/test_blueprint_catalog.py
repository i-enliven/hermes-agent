...
class TestValidation:
    ...
    def test_deliver_slot_accepts_any_platform(self):
        ...
        spec = fill_blueprint(get_blueprint("morning-brief"), {"time": "08:00", "deliver": "teams"})
        assert spec["deliver"] == "teams"
    ...
    def test_origin_threads_through(self):
        spec = fill_blueprint(
            get_blueprint("morning-brief"), {"time": "08:00"}, origin={"platform": "email", "chat_id": "9"}
        )
        assert spec["origin"] == {"platform": "email", "chat_id": "9"}
...
class TestCommandHandler:
    ...
    def test_fill_creates_job(self, isolated_home):
        ...
        res = handle_blueprint_command("morning-brief time=07:30 deliver=teams")
        assert "Scheduled" in res.text
        assert res.agent_seed is None
        jobs = isolated_home.load_jobs()
        assert len(jobs) == 1
        assert (jobs[0].get("schedule_display") or jobs[0].get("schedule")) == "30 7 * * *"
        assert jobs[0].get("deliver") == "teams"


# NOTE: TestDocsGenerator (website/scripts/extract-automation-blueprints.py)
# was removed with the fork: the entire website/ tree was deleted in the
# platform-removal baseline, so the docs generator no longer exists to test.
