...
class TestResolveDeliveryTarget:
    ...
    def test_list_form_deliver_is_normalized(self, monkeypatch):
        """deliver=['email'] (Python list) should resolve like 'email' string.
        ...
        resolved for deliver=['email']" because ``str(['email'])`` was
        passed through to ``split(',')`` verbatim.
        """
        monkeypatch.setenv("EMAIL_HOME_ADDRESS", "-4004")
        job = {
            "deliver": ["email"],
            "origin": None,
        }
...
class TestRoutingIntents:
    ...
    def test_all_expands_to_every_connected_home_channel(self, monkeypatch):
        ...
        monkeypatch.setenv("EMAIL_HOME_ADDRESS", "owner@example.com")
        ...
        assert "teams" not in platforms

        # A plugin platform with a configured home channel joins the expansion.
        monkeypatch.setenv("TEAMS_HOME_CHANNEL", "team-chat-1")
        targets = _resolve_delivery_targets({"deliver": "all", "origin": None})
        platforms = sorted(t["platform"] for t in targets)
...
class TestDeliverResultWrapping:
    ...
    def test_relay_fronted_home_uses_relay_config_and_live_adapter(self, monkeypatch, tmp_path):
        """Persisted Teams home survives restart without native Teams config."""
        from concurrent.futures import Future

        from gateway.config import GatewayConfig, HomeChannel, Platform, PlatformConfig

        relay = MagicMock()
        relay.fronts_platform.side_effect = lambda platform: platform == Platform("teams")
        ...
        monkeypatch.setenv("TEAMS_HOME_CHANNEL", "D123")
        job = {
            "id": "relay-cron",
            "deliver": "teams",
        }
        ...
        assert args[:3] == (Platform("teams"), "D123", "scheduled result")
        ...
        assert media_metadata["_relay_logical_platform"] == "teams"
    ...
    def test_live_adapter_sends_media_as_attachments(self, tmp_path, monkeypatch):
        ...
        job = {
            "id": "tts-job",
            "deliver": "origin",
            "origin": {"platform": "email", "chat_id": "9876"},
        }
...
class TestCronDeliveryTargets:
    ...
    def test_lists_configured_platforms_flagging_missing_home_channel(self, monkeypatch):
        ...
        self._patch_connected(monkeypatch, ["email", "teams"])
        monkeypatch.delenv("EMAIL_HOME_ADDRESS", raising=False)
        monkeypatch.delenv("TEAMS_HOME_CHANNEL", raising=False)

        targets = {t["id"]: t for t in cron_delivery_targets()}

        assert set(targets) == {"email", "teams"}
        # Configured but no home channel → surfaced, flagged for the UI.
        assert targets["email"]["home_target_set"] is False
        assert targets["email"]["home_env_var"] == "EMAIL_HOME_ADDRESS"
        assert targets["teams"]["home_target_set"] is False
    ...
    def test_mirror_writes_user_role_with_label_not_assistant(self):
        ...
        with patch("gateway.mirror.mirror_to_session", return_value=True) as m:
            _maybe_mirror_cron_delivery(
                {"id": "j1", "name": "Morning Brief"}, "email", "123",
    ...
    def _run_inchannel_delivery(self, extra, adapter, *, mirror_ok=True, origin=None):
        ...
        with patch("gateway.config.load_gateway_config", return_value=mock_cfg), \
            ...
            _deliver_result(
                ...
                adapters={Platform.EMAIL: adapter}, loop=loop,
            )
    ...
    def test_seed_channel_session_key_matches_inbound_channel_reply(self):
        ...
        with patch("gateway.mirror.mirror_to_session", return_value=True) as mirror_mock:
            ok = _seed_cron_channel_session(
                {"id": "j1", "name": "Brief"}, adapter, "email", "C123",
        ...
        inbound = SessionSource(
            platform=Platform.EMAIL, chat_id="C123", chat_type="group",
