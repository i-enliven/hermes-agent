...
def hermes_env(tmp_path, monkeypatch):
    ...
    # The provider/model-drift guard (#44585) snapshots the GLOBAL inference
    # config at job creation and fails closed if it differs at fire time.
    # The run_job tests stub resolve_runtime_provider to a "test" provider for
    # the fire-time agent path, so a real creation-time snapshot would drift
    # and skip every run. Clear the snapshot (no_agent-equivalent: the guard
    # never engages) — the guard is orthogonal to the monitor feature.
    import cron.jobs
    monkeypatch.setattr(cron.jobs, "_compute_provider_model_snapshots",
