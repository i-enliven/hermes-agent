...
class TestMissingLogNote:
    """A missing log file reports a plain absence.

    `hermes debug share` runs on the backend, so a log that only exists on
    the client machine can never be contributed by the backend. Reporting a
    bare absence is the only fact the backend can establish.
    """
...
class TestRunDebugShare:
    ...
    def test_share_uploads_four_pastes(self, hermes_home, capsys):
        """Successful share uploads report + agent.log + gateway.log + gui.log."""
        from hermes_cli.debug import run_debug_share
        ...
        # Should have 4 uploads: report, agent.log, gateway.log, gui.log
