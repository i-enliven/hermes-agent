...
def test_excluded_providers_hides_builtin_row(monkeypatch):
    ...
    monkeypatch.setenv("COPILOT_GITHUB_TOKEN", "gho_test_key_12345")

    baseline = list_authenticated_providers(
        current_provider="copilot",
        current_base_url="https://models.inference.ai.azure.com",
        user_providers={},
        custom_providers=[],
        max_models=50,
    )
    assert any(p["slug"] == "copilot" for p in baseline), (
        "sanity: copilot row must appear when COPILOT_GITHUB_TOKEN is set"
    )
    ...
    assert not any(p["slug"] == "copilot" for p in filtered), (
        "excluded_providers=['copilot'] must hide the copilot row"
    )
