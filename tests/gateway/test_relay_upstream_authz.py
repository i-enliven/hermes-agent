...
def test_non_upstream_adapter_still_default_denies(monkeypatch):
    ...
    runner, _ = _make_runner(platform=Platform.EMAIL, authorization_is_upstream=False)
    src = SessionSource(
...
# UNDERLYING platform (source.platform == Platform.EMAIL), not Platform.RELAY,
...
def test_relay_message_with_underlying_discord_platform_authorized(monkeypatch):
    ...
    src = SessionSource(
        platform=Platform.EMAIL,  # underlying platform off the wire
