...
def test_does_not_resurrect_disconnected_platforms_from_session_history(tmp_path, monkeypatch):
    ...
    with patch("gateway.channel_directory._build_from_sessions", side_effect=fake_build_from_sessions), \
        ...
        # Only email is connected; no telegram/discord/slack adapters.
        directory = asyncio.run(build_channel_directory({Platform.EMAIL: object()}))

    plats = directory["platforms"]
    assert "email" in plats
    # Disconnected (removed) platforms must not appear via session discovery.
    for stale in ("telegram", "discord", "slack", "whatsapp", "signal", "matrix"):
        assert stale not in plats, f"{stale} resurrected from session history"
    assert set(calls) <= {"email"}
