"""Tests for the channel session-continuity hint (Slack/Discord).

Salvaged from PR #36220 (metamon-p), ported onto the current SessionStore.

Slack and Discord were removed from this fork, so ``build_channel_continuity_note``
is now a no-op that always returns ``None``. These tests pin:
...
- build_channel_continuity_note() stays silent (returns None) for retained platforms.
"""
...
class TestBuildChannelContinuityNote:
    def test_retained_platform_stays_silent(self):
        # Slack/Discord continuity was removed with those platforms; the note
        # must be a no-op (None) even for a retained platform with a reset.
        entry = _reset_entry(Platform.EMAIL)
        assert build_channel_continuity_note(entry, _slack_source()) is None


    def test_no_activity_returns_none(self):
        entry = _reset_entry(Platform.EMAIL, had_activity=False)
        assert build_channel_continuity_note(entry, _slack_source()) is None
