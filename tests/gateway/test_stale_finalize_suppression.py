...
A *successful* finalize edit can carry only the last streamed
...
async def _run_streaming_turn(monkeypatch, tmp_path, agent_cls, session_id):
    ...
    # (_TIER_MINIMAL in gateway/display_config.py). These tests exercise
    # generic streaming-consumer behavior, so force streaming on for the
    # platform via the per-platform display override
    # (display.platforms.email.streaming, consumed at gateway/run.py:5038-5049)
    # so the stream consumer is created. Note: the global ``display.streaming``
    # key is ignored for gateway streaming (CLI-only) — only the per-platform
    # override here takes effect.
    ...
    result = await runner._run_agent(
        ...
        session_key="agent:main:email:group:-1001",
    )
...
async def test_payload_less_split_does_not_suppress_complete_response(
    ...
    result = await runner._run_agent(
        ...
        session_key="agent:main:email:group:-1004492624436:1",
    )
