...
class _DummyAdapter(BasePlatformAdapter):
    ...
    def __init__(self, platform: Platform = Platform.EMAIL):
...
def _make_event(platform: Platform = Platform.EMAIL) -> MessageEvent:
    return MessageEvent(
...
def _stream_event():
    return MessageEvent(
        text="send it again",
        message_type=MessageType.TEXT,
        source=SessionSource(platform=Platform.EMAIL, chat_id="C1", chat_type="group"),
        message_id="171.1",
    )
