...
def _make_runner(*, platform_extra: dict | None = None,
    ...
    session_entry = SessionEntry(
        session_key="agent:main:email:dm:c1",
        session_id="sess-1",
...
# ---------------------------------------------------------------------------
# Multi-platform isolation — gating on Email doesn't leak to Local.
# ---------------------------------------------------------------------------


@pytest.mark.asyncio
async def test_gating_isolated_per_platform():
    """When Email is gated and Local isn't, the same user_id on
    Local must be unrestricted."""
    from gateway.run import GatewayRunner
    from gateway.config import GatewayConfig, Platform, PlatformConfig
    ...
    runner.adapters = {
        Platform.EMAIL: MagicMock(send=AsyncMock()),
        Platform.LOCAL: MagicMock(send=AsyncMock()),
    }
    runner._voice_mode = {}
    runner.hooks = SimpleNamespace(
        emit=AsyncMock(),
    ...
    runner.session_store = MagicMock()
    session_entry = SessionEntry(
        session_key="agent:main:local:dm:c1",
        session_id="sess-1",
        created_at=datetime.now(),
        updated_at=datetime.now(),
    ...
    # Same user_id on Local → must be unrestricted (Local has no admin list).
    tg_src = _make_source(platform=Platform.LOCAL, user_id="999", chat_id="t1")
