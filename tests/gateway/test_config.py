    ...
    def test_returns_enabled_with_token(self):
        config = GatewayConfig(
            platforms={
                ...
                Platform.RELAY: PlatformConfig(enabled=True),  # no token, no relay_url
            },
        )
        connected = config.get_connected_platforms()
        assert Platform.EMAIL in connected
        assert Platform.API_SERVER not in connected
        assert Platform.RELAY not in connected
...
class TestGatewayConfigRoundtrip:
    ...
    def test_roundtrip_preserves_unauthorized_dm_behavior(self):
        ...
        assert restored.platforms[Platform.EMAIL].extra["unauthorized_dm_behavior"] == "pair"
...
class TestLoadGatewayConfig:
    ...
    def test_typing_status_text_from_nested_platforms_block(self, tmp_path, monkeypatch):
        """``platforms.email.typing_status_text`` reaches PlatformConfig via
        ...
        assert (
            config.platforms[Platform.EMAIL].typing_status_text == "chasing yarn…"
        )
    ...
    def test_bridges_nested_gateway_platforms_allowed_users_into_extra(self, tmp_path, monkeypatch):
        """gateway.platforms.<name>.extra.* must be merged into PlatformConfig.extra
        via _merge_platform_map so config.yaml extra settings reach the adapter.
        """
        hermes_home = tmp_path / ".hermes"
        hermes_home.mkdir()
        config_path = hermes_home / "config.yaml"
        ...
        assert config.platforms[Platform.EMAIL].extra["allowed_users"] == [
            "user-id-1",
            "user-id-2",
        ]
    ...
    def test_top_level_platforms_override_nested_gateway_platforms(self, tmp_path, monkeypatch):
        ...
        assert email.token == "top-token"
        assert email.extra["reply_prefix"] == "top"
    ...
    def test_shared_key_loop_bridges_allow_from_from_nested_platforms(self, tmp_path, monkeypatch):
        ...
        under ``platforms:`` (no top-level ``email:`` block).

        Before the fix, ``platform_cfg = yaml_cfg.get('email')`` returned
        ...
        email = config.platforms[Platform.EMAIL]
        assert email.extra.get("allow_from") == ["111222333", "444555666"], (
            "allow_from configured under platforms.email must be bridged "
            "into PlatformConfig.extra by the shared-key loop"
        )
        assert email.extra.get("require_mention") is True, (
            "require_mention configured under platforms.email must be "
    ...
    def test_bridges_unauthorized_dm_behavior_from_config_yaml(self, tmp_path, monkeypatch):
        ...
        assert config.unauthorized_dm_behavior == "ignore"
        assert config.platforms[Platform.EMAIL].extra["unauthorized_dm_behavior"] == "pair"


    def test_loads_email_rich_messages_from_gateway_platform_extra(self, tmp_path, monkeypatch):
        hermes_home = tmp_path / ".hermes"
        hermes_home.mkdir()
        config_path = hermes_home / "config.yaml"
        config_path.write_text(
        ...
        assert config.platforms[Platform.EMAIL].extra["rich_messages"] is False
