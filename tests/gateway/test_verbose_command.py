...
def _make_event(text="/verbose", platform=Platform.LOCAL, user_id="12345", chat_id="67890"):
...
class TestVerboseCommand:
    ...
    async def test_enabled_cycles_mode(self, tmp_path, monkeypatch):
        ...
        assert "cli" in result.lower()  # per-platform feedback (LOCAL maps to "cli")

        # Verify config was saved to display.platforms.cli
        saved = yaml.safe_load(config_path.read_text(encoding="utf-8"))
        assert saved["display"]["platforms"]["cli"]["tool_progress"] == "verbose"
