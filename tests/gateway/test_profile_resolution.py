...
class TestNonDiscordProfileRouting:
    ...
    def test_no_route_match_preserves_default_sentinel(self, mock_runner, telegram_source):
        ...
        adapter = _stub_adapter(Platform.EMAIL, mock_runner)
...
class TestAdapterToSessionKeyIntegration:
    ...
    The review asked for integration coverage for a chat platform (LOCAL,
    the retained pair-default chat platform in this fork) AND a non-chat
    ...
    shared ``agent:main:...``). The LOCAL case is the regression guard:
    ...
    def _routes():
        return [
            ProfileRoute(name="dc", platform="local", profile="coder",
                         guild_id="111", chat_id="222"),
            ProfileRoute(name="tg", platform="local", profile="ops",
                         chat_id="-1001234567890"),
        ]

    def test_local_adapter_stamps_profile_and_scopes_key(self, mock_runner):
        mock_runner.config.profile_routes = self._routes()
        adapter = _stub_adapter(Platform.LOCAL, mock_runner)
    ...
    async def test_direct_source_is_rejected_at_shared_ingress(self, mock_runner):
        ...
        source = SessionSource(platform=Platform.LOCAL, chat_id="route-chat")
