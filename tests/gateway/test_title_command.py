...
def _make_event(text="/title", platform=Platform.LOCAL,
...
def _make_runner(session_db=None):
    ...
    mock_session_entry.session_key = "local:12345:67890"
...
class TestHandleTitleCommand:
    ...
    async def test_title_conflict(self, tmp_path):
        ...
        db.create_session("other_session", "local")
        db.set_session_title("other_session", "Taken Title")
        db.create_session("test_session_123", "local")
...
class TestResetCommandWithTitle:
    ...
    async def test_reset_command_duplicate_title_surfaces_warning(self):
        ...
        runner.config = GatewayConfig(
            platforms={Platform.LOCAL: PlatformConfig(enabled=True, token="***")}
        )
        adapter = MagicMock()
