...
async def test_full_dispatch_rejects_lease_timeout_without_running_goal_hook(
    ...
    try:
        # outer bound widened for load-robustness (flake 2026-08-19 under
        # -j 40): the assertion is on the REJECTION OUTCOME, not on latency;
        # HERMES_TURN_LEASE_TIMEOUT=0.02 still proves the lease wait rejected
        # promptly in principle — the 1s outer bound flaked when the default
        # thread pool was starved by 40 concurrent pytest processes.
