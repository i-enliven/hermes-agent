...
# ===================================================================
# Test 3: Cross-platform delivery (webhook → email)
# ===================================================================
...
class TestCrossPlatformDelivery:
    ...
    async def test_cross_platform_delivery(self):
        """When deliver='email', the response is routed to the
        email adapter via gateway_runner.adapters."""
        routes = {
            "alerts": {
                "secret": _INSECURE_NO_AUTH,
        ...
        # Set up a mock gateway runner with a mock email adapter
        mock_email_adapter = AsyncMock()
        mock_email_adapter.send = AsyncMock(return_value=SendResult(success=True))

        mock_runner = MagicMock()
        mock_runner.adapters = {Platform.EMAIL: mock_email_adapter}
        mock_runner.config = GatewayConfig(
            platforms={Platform.EMAIL: PlatformConfig(enabled=True, token="fake")}
        )
        ...
        mock_email_adapter.send.assert_awaited_once_with(
