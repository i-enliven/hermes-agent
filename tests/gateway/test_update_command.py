...
def _make_event(text="/update", platform=Platform.EMAIL,
...
class TestHandleUpdateCommand:
    ...
    async def test_writes_pending_marker(self, tmp_path):
        ...
        runner = _make_runner()
        event = _make_event(platform=Platform.EMAIL, chat_id="99999")
...
class TestSendUpdateNotification:
    ...
    async def test_defers_notification_while_update_still_running(self, tmp_path):
        ...
        pending_path.write_text(json.dumps({
            "platform": "email", "chat_id": "67890", "user_id": "12345",
        }))
        (hermes_home / ".update_output.txt").write_text("still running")

        mock_adapter = AsyncMock()
        runner.adapters = {Platform.EMAIL: mock_adapter}
    ...
    async def test_cleans_up_on_error(self, tmp_path):
        ...
        pending_path.write_text(json.dumps({
            "platform": "email", "chat_id": "111", "user_id": "222",
        }))
    ...
    async def test_no_adapter_for_platform_preserves_markers(self, tmp_path):
        ...
        pending = {"platform": "teams", "chat_id": "111", "user_id": "222"}
        pending_path = hermes_home / ".update_pending.json"
        output_path = hermes_home / ".update_output.txt"
        exit_code_path = hermes_home / ".update_exit_code"
        ...
        # Only email adapter available, but pending says teams
        mock_adapter = AsyncMock()
        runner.adapters = {Platform.EMAIL: mock_adapter}
    ...
    async def test_deferred_notification_delivers_after_reconnect(self, tmp_path):
        ...
        # First pass: target platform (teams) is still offline → defer.
        with patch("gateway.run._hermes_home", hermes_home):
        ...
        mock_adapter = AsyncMock()
        runner.adapters = {Platform("teams"): mock_adapter}
