...
def _record(oid="ob-1", session_key="agent:main:email:channel:C1", **kw):
    dl.record_obligation(
        obligation_id=oid,
        session_key=session_key,
        platform=kw.get("platform", "email"),
...
class TestGatewayRedeliverySweep:
    ...
    def _runner(adapter=None):
        ...
        runner.adapters = {Platform.EMAIL: adapter} if adapter else {}
        _store = MagicMock()
...
class TestAttemptsOnlySpentOnRealSends:
    ...
    def test_row_still_delivers_once_its_platform_returns(self):
        ...
        for _ in range(dl.MAX_ATTEMPTS + 2):
            _orphan("ob-1")
            dl.sweep_recoverable(deliverable_platforms={"webhook"})

        _orphan("ob-1")
        claimed = dl.sweep_recoverable(deliverable_platforms={"email"})
        assert len(claimed) == 1
        assert claimed[0]["attempts"] == 1
...
class TestUnconnectedPlatformKeepsItsBudget:
    ...
    @staticmethod
    def _runner_without_email():
        from gateway.run import GatewayRunner

        runner = object.__new__(GatewayRunner)
        runner.adapters = {}  # email failed to connect this boot
        _store = MagicMock()
        _store.clear_resume_pending = AsyncMock()
        _store._store = None
        runner.session_store = None
    ...
    async def test_row_survives_boots_where_its_platform_is_down(self):
        ...
        for _ in range(dl.MAX_ATTEMPTS + 1):
            _orphan("ob-1")
            runner = self._runner_without_email()
