...
def _create_completed_subscription(summary="done once"):
    ...
    try:
        ...
        kb.add_notify_sub(conn, task_id=tid, platform="email", chat_id="chat-1")
...
def test_kanban_notifier_replays_email_dm_topic_delivery_metadata(tmp_path, monkeypatch):
    ...
    try:
        tid = kb.create_task(
            ...
            title="dm topic task",
            assignee="worker",
            session_id="agent:main:email:dm:chat-1",
        )
        kb.add_notify_sub(
            conn,
            task_id=tid,
...
def test_notifier_wakeup_uses_subscription_chat_type(tmp_path, monkeypatch):
    ...
    assert wake_key == "agent:main:email:dm:chat-dm"
...
def test_kanban_notifier_isolates_per_subscription_failure(tmp_path, monkeypatch):
    ...
    try:
        ...
        kb.add_notify_sub(conn, task_id=tid_bad, platform="email", chat_id="chat-bad")
        kb.complete_task(conn, tid_bad, summary="done")

        tid_good = kb.create_task(conn, title="good task", assignee="worker")
        kb.add_notify_sub(conn, task_id=tid_good, platform="email", chat_id="chat-good")
...
def test_notifier_delivers_block_loop_detected_triage_ping(tmp_path, monkeypatch):
    ...
    try:
        _, remaining = kb.unseen_events_for_sub(
            conn, task_id=tid, platform="email", chat_id="chat-1",
