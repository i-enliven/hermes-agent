"""Tests for confirmed-dead delivery-target short-circuiting (deleted
...
class TestDeadTargetRegistry:
    def test_mark_is_dead_clear_roundtrip(self, isolate):
        ...
        assert reg.is_dead("email", "123") is False
        assert reg.mark_dead("email", "123", "forbidden") is True
        assert reg.is_dead("email", "123") is True
        # idempotent: second mark returns False (already present)
        assert reg.mark_dead("email", "123", "forbidden") is False
        assert reg.clear("email", "123") is True
        assert reg.is_dead("email", "123") is False

    def test_persists_across_instances(self, isolate):
        reg = DeadTargetRegistry()
        reg.mark_dead("email", "999", "deleted group")
        # New instance reads the same on-disk store under tmp HERMES_HOME.
        reg2 = DeadTargetRegistry()
        assert reg2.is_dead("email", "999") is True
...
async def test_forbidden_marks_target_dead_then_short_circuits(isolate):
    ...
    target = DeliveryTarget.parse("email:42")

    # First delivery: send raises Forbidden -> failure + target recorded dead.
    res1 = await router.deliver("hi", [target])
    assert res1["email:42"]["success"] is False
    assert router.dead_targets.is_dead("email", "42") is True
    ...
    assert res2["email:42"]["skipped"] == "dead_target"
    assert res2["email:42"]["success"] is False
...
async def test_shared_registry_is_used_when_injected(isolate):
    ...
    shared.mark_dead("email", "500", "pre-existing")
