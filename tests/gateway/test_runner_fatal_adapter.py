...
class _FatalAdapter(BasePlatformAdapter):
    def __init__(self):
        super().__init__(PlatformConfig(enabled=True, token="token"), Platform.EMAIL)
...
async def test_runner_queues_retryable_runtime_fatal_for_reconnection(monkeypatch, tmp_path):
    ...
    config = GatewayConfig(
        platforms={
            Platform.EMAIL: PlatformConfig(enabled=True, token="token")
        },
    ...
    assert runner._failed_platforms[Platform.EMAIL]["attempts"] == 0
...
async def test_retryable_fatal_queues_reconnect_after_cancellation_swallowing_disconnect(
    ...
    config = GatewayConfig(
        platforms={Platform.EMAIL: PlatformConfig(enabled=True, token="token")},
        sessions_dir=tmp_path / "sessions",
    )
    runner = GatewayRunner(config)
    adapter = _RuntimeRetryableAdapter()
