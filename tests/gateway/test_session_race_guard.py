...
def _make_runner():
    ...
    runner.config = GatewayConfig(
        platforms={Platform.EMAIL: PlatformConfig(enabled=True, token="***")}
    )
    runner.adapters = {Platform.EMAIL: _FakeAdapter()}
...
def _make_event(text="hello", chat_id="12345"):
    source = SessionSource(
        platform=Platform.EMAIL, chat_id=chat_id, chat_type="dm",
        user_id="u1",
    )
...
async def test_start_command_is_noop_during_active_session():
    ...
    assert session_key not in runner.adapters[Platform.EMAIL]._pending_messages
