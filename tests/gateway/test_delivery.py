...
class TestParseTargetPlatformChat:
    def test_explicit_email_chat(self):
        target = DeliveryTarget.parse("email:12345")
        assert target.platform == Platform.EMAIL
        assert target.chat_id == "12345"
        assert target.is_explicit is True


    def test_origin_with_source(self):
        origin = SessionSource(platform=Platform.EMAIL, chat_id="789", thread_id="42")
...
class TestTargetToStringRoundtrip:
    def test_origin_roundtrip(self):
        origin = SessionSource(platform=Platform.EMAIL, chat_id="111", thread_id="42")
        target = DeliveryTarget.parse("origin", origin=origin)
...
class TestCaseSensitiveChatIdParsing:
    ...
    def test_email_uppercase_chat_id_preserved(self):
        """Channel IDs like C123ABC should preserve case."""
        target = DeliveryTarget.parse("email:C123ABC")
        assert target.platform == Platform.EMAIL
...
class _RelayDeliveryTransport:
    """Relay transport that advertises email and records outbound wire frames."""

    def __init__(self):
        self._identities = [("email", "bot-1")]
        self.sent = []
...
async def test_relay_fronted_target_delivers_without_prior_inbound_chat_state(tmp_path, monkeypatch):
    """A persisted email home must work immediately after a gateway restart."""
    ...
    result = await router._deliver_to_platform(
        DeliveryTarget(platform=Platform.EMAIL, chat_id="D123"),
        "scheduled result",
...
async def test_native_adapter_wins_when_relay_also_fronts_platform(tmp_path, monkeypatch):
    ...
    router = DeliveryRouter(
        config,
        adapters={Platform.EMAIL: native, Platform.RELAY: relay},
    )
...
async def test_disabled_native_adapter_does_not_shadow_relay(tmp_path, monkeypatch):
    ...
    assert native.calls == []
    assert len(transport.sent) == 1
    assert transport.sent[0][1] == "email"
...
class ChunkingAdapter:
    """Adapter that declares splits_long_messages=True (native chunking)."""
...
async def test_long_output_truncated_for_non_chunking_adapter(tmp_path, monkeypatch):
    ...
    target = DeliveryTarget.parse("email:123")
