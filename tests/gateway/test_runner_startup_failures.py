...
class _RetryableFailureAdapter(BasePlatformAdapter):
    def __init__(self):
        super().__init__(PlatformConfig(enabled=True, token="***"), Platform.EMAIL)
...
class _DisabledAdapter(BasePlatformAdapter):
    def __init__(self):
        super().__init__(PlatformConfig(enabled=False, token="***"), Platform.EMAIL)
...
async def test_runner_degrades_gracefully_when_all_adapters_missing(monkeypatch, tmp_path, caplog):
    ...
    config = GatewayConfig(
        platforms={
            Platform.EMAIL: PlatformConfig(enabled=True, token="***"),
            Platform.EMAIL: PlatformConfig(enabled=True, token="***"),
        },
