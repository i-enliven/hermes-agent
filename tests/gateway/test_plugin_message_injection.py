...
def _entry(*, origin=True) -> SessionEntry:
    ...
    now = datetime.now()
    return SessionEntry(
        session_key="agent:main:email:dm:42",
...
async def test_dispatch_rejects_stored_role_only_authorization(monkeypatch):
    ...
    entry = _entry()
    entry.session_key = "agent:main:email:dm:42"
    entry.platform = Platform.EMAIL
    source = entry.origin
...
async def test_base_adapter_rejects_derived_session_mismatch():
    ...
    event = MessageEvent(
        ...
        internal=True,
        allow_gateway_control=False,
        metadata={"gateway_session_key": "agent:main:email:dm:other"},
    )
