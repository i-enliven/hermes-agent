...
class TestStartupPlatformIsolation:
    ...
    async def test_start_continues_after_platform_connect_timeout(self, tmp_path):
        """A timeout on Email should queue it and still connect Local."""
        runner = _make_runner()
        runner.config = GatewayConfig(
            platforms={
                Platform.EMAIL: PlatformConfig(enabled=True, token="test"),
                Platform.LOCAL: PlatformConfig(enabled=True, token="test"),
            },
        ...
        adapters = {
            Platform.EMAIL: StubAdapter(platform=Platform.EMAIL),
            Platform.LOCAL: StubAdapter(platform=Platform.LOCAL),
        }
        ...
        runner._connect_adapter_with_timeout = AsyncMock(
            side_effect=[
                TimeoutError("email connect timed out after 30s"),
                True,
            ]
        )
        ...
        assert Platform.LOCAL in runner.adapters
...
class TestStartupFailureQueuing:
    ...
    def test_failed_platform_queued_on_connect_failure(self):
        ...
        runner._failed_platforms[Platform.EMAIL] = {
            "config": platform_config,
            "attempts": 1,
            "next_retry": time.monotonic() + 30,
        }
        assert Platform.EMAIL in runner._failed_platforms
        assert runner._failed_platforms[Platform.EMAIL]["attempts"] == 1
...
class TestPlatformReconnectWatcher:
    ...
    async def test_reconnect_passes_is_reconnect_true(self):
        ...
        assert Platform.EMAIL in runner.adapters
    ...
    async def test_cold_connect_defaults_to_is_reconnect_false(self):
        ...
        success = await runner._connect_adapter_with_timeout(adapter, Platform.EMAIL)
    ...
    async def test_reconnect_never_auto_pauses_retryable_failures(self):
        ...
        assert Platform.EMAIL in runner._failed_platforms
        info = runner._failed_platforms[Platform.EMAIL]
...
class TestPauseResume:
    ...
    def test_pause_is_idempotent(self):
        ...
        runner._pause_failed_platform(Platform.EMAIL, reason="second reason")
        # Reason should not be overwritten on a second pause call.
        assert (
            runner._failed_platforms[Platform.EMAIL]["pause_reason"]
            == "first reason"
        )
    ...
    def test_resume_clears_paused_and_resets_attempts(self):
        ...
        assert runner._resume_paused_platform(Platform.EMAIL) is True
        info = runner._failed_platforms[Platform.EMAIL]
...
class TestPlatformSlashCommand:
    ...
    async def test_list_shows_connected_and_paused(self):
        runner = _make_runner()
        runner.adapters[Platform.EMAIL] = StubAdapter(platform=Platform.EMAIL)
        runner._failed_platforms[Platform.LOCAL] = {
            "config": PlatformConfig(enabled=True, token="t"),
            "attempts": 10,
            "next_retry": float("inf"),
            "paused": True,
    ...
    async def test_pause_command_pauses_queued_platform(self):
        ...
        out = await runner._handle_platform_command(
            self._make_event("/platform pause local")
        )
        assert "paused" in out.lower()
        assert runner._failed_platforms[Platform.LOCAL]["paused"] is True
...
class TestFatalHandoffCancellationProof:
    ...
    async def test_stranded_retryable_platform_exits_for_supervisor_restart(self):
        ...
        runner.adapters[Platform.LOCAL] = StubAdapter(platform=Platform.LOCAL)
...
class TestReconnectWatcherRaceGuard:
    ...
    async def test_retryable_fatal_error_calls_ensure_watcher(self):
        ...
        platform_config = PlatformConfig(enabled=True, token="test")
        runner.config = GatewayConfig(
            platforms={Platform.EMAIL: platform_config}
        )
...
class TestVoiceInputCallbackWiring:
    """Startup and reconnect must wire _voice_input_callback on the adapter."""

    @staticmethod
    def _make_email_voice_adapter():
        """A minimal adapter stub with voice attributes."""
        adapter = MagicMock()
    ...
    def _make_runner_with_email(self):
        runner = _make_runner()
        runner.config = GatewayConfig(
            platforms={Platform.EMAIL: PlatformConfig(enabled=True, token="test")}
        )
    ...
    async def test_startup_wires_voice_input_callback(self, tmp_path):
        """Cold-start connect must wire _voice_input_callback on the adapter."""
        runner = self._make_runner_with_email()
        adapter = self._make_email_voice_adapter()
