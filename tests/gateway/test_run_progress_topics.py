...
class ProgressCaptureAdapter(BasePlatformAdapter):
    def __init__(self, platform=Platform.EMAIL):
...
class UrlPreviewAgent:
    URL = "https://hermes-agent.nousresearch.com/docs/gateway/tool-progress"
...
def _run_long_preview_helper(monkeypatch, tmp_path, preview_length=0):
    ...
    result = asyncio.get_event_loop().run_until_complete(
        runner._run_agent(
            ...
            session_key="agent:main:email:dm:12345",
        )
    )
...
async def test_transformed_response_edits_streamed_message_in_place(monkeypatch, tmp_path):
    ...
    adapter, result = await _run_with_agent(
        ...
        config_data={
            # EMAIL's display tier disables gateway token streaming by default
            # (_TIER_MINIMAL in gateway/display_config.py); this generic
            # streaming test needs the consumer, so force it on via the
            # per-platform display override (run.py:5038-5049).
            "display": {
                "tool_progress": "off",
                "interim_assistant_messages": False,
        ...
        platform=Platform.EMAIL,
        chat_id="!room:email.example.org",
        chat_type="group",
        thread_id="$thread",
        ...
        "image_batches": [
            {
                ...
                "metadata": {"thread_id": "email-thread"},
            }
        ],
    }
...
async def test_base_processing_releases_post_delivery_callback_after_main_send():
    ...
    session_key = "agent:main:email:group:-1001:17585"
...
async def test_run_agent_drops_tool_progress_after_generation_invalidation(monkeypatch, tmp_path):
    ...
    adapter = ProgressCaptureAdapter(platform=Platform.EMAIL)
    runner = _make_runner(adapter)
    gateway_run = importlib.import_module("gateway.run")
    monkeypatch.setattr(gateway_run, "_hermes_home", tmp_path)
    monkeypatch.setattr(gateway_run, "_resolve_runtime_agent_kwargs", lambda: {"api_key": "***"})
    ...
    session_key = "agent:main:email:dm:dm-1"
...
async def test_run_agent_drops_interim_commentary_after_generation_invalidation(monkeypatch, tmp_path):
    ...
    session_key = "agent:main:email:dm:dm-2"
    runner._session_run_generation[session_key] = 1
...
async def test_terminal_progress_renders_fenced_code_block(monkeypatch, tmp_path):
    ...
    renders a bare fenced code block — no language tag
    ...
    adapter = CodeBlockProgressAdapter(platform=Platform.EMAIL)
    runner = _make_runner(adapter)
    gateway_run = importlib.import_module("gateway.run")
    monkeypatch.setattr(gateway_run, "_hermes_home", tmp_path)
    monkeypatch.setattr(gateway_run, "_resolve_runtime_agent_kwargs", lambda: {"api_key": "***"})
