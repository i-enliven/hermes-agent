...
def _make_event(text: str = "hello", platform: Platform = Platform.EMAIL) -> MessageEvent:
    return MessageEvent(
        text=text,
        message_id="m1",
        source=SessionSource(
            platform=platform,
            user_id="15551234567@s.email.test",
            chat_id="15551234567@s.email.test",
            user_name="tester",
            chat_type="dm",
        ),
    )
...
async def test_internal_events_bypass_hook(monkeypatch):
    ...
    runner, _adapter = _make_runner(Platform.EMAIL)
...
async def test_hook_fires_without_session_store_attribute(monkeypatch):
    ...
    runner, adapter = _make_runner(Platform.EMAIL)
