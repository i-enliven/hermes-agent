...
Salvaged and de-scoped from a superseded platform-specific PR.
"""
...
@pytest.mark.parametrize("platform", [Platform.EMAIL, Platform.EMAIL])
class TestExtractStripRecoveryAllPlatforms:
    """A non-empty response stripped to empty must be recovered on EVERY
    platform (the fix de-scopes the recovery from platform-specific)."""
...
class TestRecoveryDoesNotLeakMediaFragments:
    ...
    async def test_spaced_media_path_does_not_leak_fragment(self, monkeypatch, caplog):
        adapter = _DummyAdapter(Platform.EMAIL)
        adapter._keep_typing = _hold_typing
        ...
        event = _make_event(Platform.EMAIL)
...
class TestPostStopInterruptSwallow:
    ...
    async def test_interrupt_and_clear_session_evicts_cached_agent(
        ...
        source = SessionSource(
            platform=Platform.EMAIL, chat_id="12345", chat_type="dm"
        )
