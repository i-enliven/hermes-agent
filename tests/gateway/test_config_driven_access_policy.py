...
Background (#34515): config-driven access surface (``dm_policy`` /
``group_policy`` / ``allow_from`` / ``group_allow_from`` in
``PlatformConfig.extra``) enforced at intake —
...
def test_open_group_with_per_group_sender_allowlist_is_authorized(monkeypatch):
    """``groups.<id>.allow_from`` is an adapter-enforced restriction.
    ...
    runner, _adapter = _make_runner(Platform.LOCAL, config, enforces=True)

    assert runner._is_user_authorized(_source(Platform.LOCAL, chat_type="group")) is True
...
# ---------------------------------------------------------------------------
#
# Regression: own-policy adapters document ``dm_policy: pairing`` and declare
...
def test_unauthorized_dm_behavior_follows_config_dm_policy(monkeypatch, dm_policy, expected):
    ...
    _clear_auth_env(monkeypatch)
    config = GatewayConfig(
        platforms={Platform.LOCAL: PlatformConfig(enabled=True, extra={"dm_policy": dm_policy})}
    )
    runner, _adapter = _make_runner(Platform.LOCAL, config, enforces=True)

    assert runner._get_unauthorized_dm_behavior(Platform.LOCAL) == expected
...
def test_unauthorized_dm_behavior_open_policy_keeps_default(monkeypatch):
    ...
    _clear_auth_env(monkeypatch)
    config = GatewayConfig(
        platforms={Platform.LOCAL: PlatformConfig(enabled=True, extra={"dm_policy": "open"})}
    )
    runner, _adapter = _make_runner(Platform.LOCAL, config, enforces=True)

    # No allowlist + no restrictive policy → open-gateway pairing default.
    assert runner._get_unauthorized_dm_behavior(Platform.LOCAL) == "pair"
