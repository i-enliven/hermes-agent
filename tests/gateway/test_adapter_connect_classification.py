...
Email used to funnel every startup failure into an indefinitely-retried
state — including permanent failures like revoked tokens, missing
privileged intents, and sidecar deps that can never install on an immutable
image. Fleet triage found agents that had been silently "retrying" for weeks
(OOF-151/152/153).
...
class TestWatcherAttentionEscalation:
    ...
    async def test_watcher_flags_long_queued_platform_and_keeps_retrying(self, monkeypatch):
        ...
        assert runner._failed_platforms[Platform.EMAIL].get("attention_flagged") is True
