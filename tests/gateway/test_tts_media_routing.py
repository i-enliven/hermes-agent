...
platforms (where Bot-API sendAudio only accepts MP3/M4A and .ogg/.opus
...
@pytest.mark.asyncio
async def test_base_adapter_routes_voice_tagged_ogg_media_tag_to_voice_sender(tmp_path, monkeypatch):
...
class _EmailMediaFailureAdapter(BasePlatformAdapter):
...
async def test_non_streaming_media_failure_notifies_user(tmp_path, monkeypatch):
    ...
    adapter = _EmailMediaFailureAdapter()
    event = _event()
    ...
    adapter.send_video = AsyncMock(
        return_value=SendResult(
            success=False,
            error="Adapter accepted the message but attached no files (clip.mp4)",
        )
    )
    adapter.send_document = AsyncMock(return_value=SendResult(success=True, message_id="doc"))
    adapter.send_voice = AsyncMock(return_value=SendResult(success=True, message_id="voice"))
