...
def _make_source(platform=Platform.EMAIL, chat_id="123", user_id="u1"):
...
class TestSessionEntryResumeFields:
    def test_defaults(self):
        now = datetime.now()
        entry = SessionEntry(
            session_key="agent:main:email:dm:1",
            session_id="sid",
            created_at=now,
            updated_at=now,
        )
...
async def test_drain_timeout_marks_resume_pending():
    ...
    session_key_one = "agent:main:email:dm:A"
    session_key_two = "agent:main:email:dm:B"
...
async def test_startup_auto_resume_skips_unauthorized_owner():
    ...
    pending_entry = SessionEntry(
        session_key="agent:main:email:dm:revoked-chat",
        session_id="sid",
...
async def test_reconnect_reschedule_is_platform_scoped():
    ...
    discord_source = SessionSource(
        platform=Platform.LOCAL, chat_id="dc-chat", chat_type="dm", user_id="u1"
    )
    tg_entry = SessionEntry(
        session_key="agent:main:email:dm:tg-chat",
        session_id="sid-tg",
        created_at=datetime.now(),
        updated_at=datetime.now(),
        origin=tg_source,
    ...
    discord_entry = SessionEntry(
        session_key="agent:main:local:dm:dc-chat",
        session_id="sid-dc",
        created_at=datetime.now(),
        updated_at=datetime.now(),
    ...
    scheduled = runner._schedule_resume_pending_sessions(platform=Platform.EMAIL)
    await asyncio.sleep(0)

    # Only the email session is resumed; the local session waits for its
    # own reconnect.
    assert scheduled == 1
...
async def test_startup_restore_waits_for_resume_before_draining_inbound():
    ...
    pending_entry = SessionEntry(
        session_key="agent:main:email:dm:restore-chat",
        session_id="sid",
        created_at=datetime.now(),
        updated_at=datetime.now(),
        origin=source,
...
async def test_restart_home_channel_notification_not_deduped_across_threads():
    runner, adapter = make_restart_runner()
    runner._restart_requested = True
    session_key = "agent:main:email:group:999"
...
async def test_auto_resume_sets_sentinel_before_task_execution():
    ...
    pending_entry = SessionEntry(
        session_key="agent:main:email:dm:race-chat",
        session_id="sid",
        created_at=datetime.now(),
        updated_at=datetime.now(),
        origin=source,
