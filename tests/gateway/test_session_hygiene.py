...
class HygieneCaptureAdapter(BasePlatformAdapter):
    def __init__(self):
        super().__init__(PlatformConfig(enabled=True, token="fake-token"), Platform.EMAIL)
        self.sent = []
...
async def test_session_hygiene_preserves_transcript_when_no_rotation(monkeypatch, tmp_path):
    ...
    runner.config = GatewayConfig(
        platforms={Platform.EMAIL: PlatformConfig(enabled=True, token="fake-token")}
    )
...
async def test_session_hygiene_timeout_continues_to_agent_and_sets_cooldown(monkeypatch, tmp_path):
    ...
    #
    # 2026-08-19: the 2.0 bound flaked under the canonical -j 40 harness
    # (40 concurrent pytest files). Measured internal elapsed of
    # await runner._handle_message(event) [the exact value this bound guards]:
    #   - clean (15 runs):           0.42-0.55s  (max 0.553s)
    #   - NPROC=20 CPU stressor:     1.21-1.85s  (max 1.854s)
    #   - 40 concurrent pytest procs: 2.08-7.38s (max 7.376s; 40/40 exceeded
    #     the OLD 2.0 bound, and 26/40 exceeded an interim 5.0 bound)
    # Root cause (instrumented via time/asyncio/threading call-site shims):
    # the hygiene slice loop fires exactly ONCE (timeout=0.0098 actual=0.0106,
    # since seconds_since_progress() > 0.01 immediately and the fake worker
    # never calls touch_progress) and the SUM of all awaits inside
    # _handle_message is only ~0.26s under load — NO single blocking call.
    # The wall-clock is ~30 tiny asyncio.sleep/wait_for/to_thread round-trips
    # each paying event-loop scheduling + GIL contention under 40x load.
    # This matches the flake-policy precedent (test_24996: "previous
    # wall-clock upper bound flaked on loaded runners").
    #
    # Bound = 15.0s = 2x my worst MEASURED loaded elapsed (7.376s) and
    # 27x the worst clean (0.55s). It stays 2x below the real 30s default
    # hygiene timeout and 40x below the 600s total ceiling — so it still
    # meaningfully catches a genuine hang while leaving real headroom for the
    # -j 40 canonical harness.
    assert elapsed < 15.0
    ...
    release_worker.set()
    # Defensive (2026-08-19, -j 40): the original
    #     await asyncio.wait_for(asyncio.to_thread(cleanup_done.wait), timeout=2)
    # ran cleanup_done.wait() on the DEFAULT asyncio thread pool, whose size is
    # min(32, cpu+4). Under the canonical -j 40 harness (40 concurrent pytest
    # files sharing a box) that pool can be starved, so to_thread can spend the
    # whole 2s budget just QUEUING for a free worker thread and raise a
    # spurious TimeoutError even though cleanup is instant. Wait on a DEDICATED
    # per-test single-thread executor instead: one reserved thread no other
    # test/producer contends for, so the wait reflects real cleanup latency,
    # not pool scheduling. (Mirrors asyncio.to_thread's run_in_executor
    # semantics; the worker thread itself is still release_worker.wait(2), its
    # own intentional 2s cap.)
    _cleanup_pool = concurrent.futures.ThreadPoolExecutor(max_workers=1)
    try:
        await asyncio.wait_for(
            asyncio.get_running_loop().run_in_executor(_cleanup_pool, cleanup_done.wait),
            timeout=2,
        )
    finally:
        # Success path: the cleanup thread has finished (run_in_executor's
        # future only resolves after the fn returns), so shutdown is instant.
        # Timeout path: the thread is left to finish on its own (it cannot
        # exceed its own 2s wait) — the same behavior as asyncio.to_thread.
        _cleanup_pool.shutdown(wait=True)
