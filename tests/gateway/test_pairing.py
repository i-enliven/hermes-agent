...
class TestSplitPairingDirMigration:
    def test_merges_new_approved_into_active_legacy_dir(self, tmp_path):
        ...
        (new / "webhook-approved.json").write_text(json.dumps({
        ...
        with patch("gateway.pairing.PAIRING_DIR", legacy), patch("gateway.pairing.get_hermes_home", return_value=home):
            store = PairingStore()
            assert store.is_approved("webhook", "ou_user") is True

        migrated = json.loads((legacy / "webhook-approved.json").read_text())
...
class TestProfileScopedDiscovery:
    def test_list_approved_scopes_platform_discovery_to_profile_dir(self, tmp_path):
        ...
        with patch("gateway.pairing.PAIRING_DIR", global_dir), patch(
            ...
            with store._lock:
                store._approve_user("email", "tg-456", "Bob")

            assert store.is_approved("email", "tg-456") is True
            approved = store.list_approved()

        assert [r["user_id"] for r in approved] == ["tg-456"]
        assert approved[0]["platform"] == "email"
...
class TestCodeGeneration:
    ...
    def test_code_uniqueness(self, tmp_path):
        ...
        with patch("gateway.pairing.PAIRING_DIR", tmp_path):
            ...
            codes = set()
            for i in range(3):
                code = store.generate_code("email", f"user{i}")
...
class TestHashedStorage:
    def test_pending_file_contains_hash_and_salt(self, tmp_path):
        ...
        with patch("gateway.pairing.PAIRING_DIR", tmp_path):
            store = PairingStore()
            code = store.generate_code("email", "user1", "Alice")
            raw = json.loads(
                (tmp_path / "email-pending.json").read_text(encoding="utf-8")
            )
    ...
    def test_plaintext_code_not_stored(self, tmp_path):
        ...
        with patch("gateway.pairing.PAIRING_DIR", tmp_path):
            store = PairingStore()
            code = store.generate_code("email", "user1")
            raw_text = (tmp_path / "email-pending.json").read_text(encoding="utf-8")
...
class TestLegacyPendingFileCompat:
    ...
    def _write_legacy(tmp_path, code="ABCD1234", created_at=None):
        ...
        (tmp_path / "email-pending.json").write_text(
    ...
    def test_approve_code_skips_malformed_entries(self, tmp_path):
        ...
        with patch("gateway.pairing.PAIRING_DIR", tmp_path):
            ...
            assert store.approve_code("email", "ABCD1234") is None
...
class TestRateLimiting:
    def test_same_user_rate_limited(self, tmp_path):
        with patch("gateway.pairing.PAIRING_DIR", tmp_path):
            store = PairingStore()
            code1 = store.generate_code("email", "user1")
            code2 = store.generate_code("email", "user1")
...
class TestApprovalFlow:
    def test_approve_valid_code(self, tmp_path):
        with patch("gateway.pairing.PAIRING_DIR", tmp_path):
            store = PairingStore()
            code = store.generate_code("email", "user1", "Alice")
            result = store.approve_code("email", code)
    ...
    def test_approved_user_is_approved(self, tmp_path):
        with patch("gateway.pairing.PAIRING_DIR", tmp_path):
            store = PairingStore()
            code = store.generate_code("email", "user1", "Alice")
            store.approve_code("email", code)
            assert store.is_approved("email", "user1") is True

    def test_approve_request_id_from_pending_list(self, tmp_path):
        with patch("gateway.pairing.PAIRING_DIR", tmp_path):
            store = PairingStore()
            bot_code = store.generate_code("email", "user1", "Alice")
            pending = store.list_pending("email")
            request_id = pending[0]["request_id"]

            assert request_id
            assert request_id != bot_code

            result = store.approve_request("email", request_id.upper())
            remaining = store.list_pending("email")
    ...
    def test_approve_request_never_reveals_or_accepts_the_code_digest(self, tmp_path):
        ...
        with patch("gateway.pairing.PAIRING_DIR", tmp_path):
            store = PairingStore()
            bot_code = store.generate_code("email", "user1", "Alice")
            entry = store.list_pending("email")[0]

            digest = json.loads(
                (tmp_path / "email-pending.json").read_text()
            ...
            assert store.approve_code("email", digest[:8]) is None
            assert store.approve_request("email", digest[:8]) is None
    ...
    def test_stale_request_id_never_locks_out_the_code_path(self, tmp_path):
        ...
        with patch("gateway.pairing.PAIRING_DIR", tmp_path):
            store = PairingStore()
            code = store.generate_code("email", "user1", "Alice")
            stale_id = store.list_pending("email")[0]["request_id"]
            assert store.approve_request("email", stale_id) is not None

            # Re-click the now-approved row well past the lockout threshold.
            for _ in range(MAX_FAILED_ATTEMPTS + 3):
                assert store.approve_request("email", stale_id) is None

            assert store._is_locked_out("email") is False
            # And the code path is still usable for the next real request.
            next_code = store.generate_code("email", "user2", "Bee")
            assert store.approve_code("email", next_code) is not None
    ...
    def test_successful_approval_resets_failure_counter(self, tmp_path):
        ...
        with patch("gateway.pairing.PAIRING_DIR", tmp_path):
            ...
            for _ in range(MAX_FAILED_ATTEMPTS - 1):
                assert store.approve_code("email", "WRONGCODE") is None
            assert store._is_locked_out("email") is False

            # A legitimate approval must reset the accumulated failures.
            code = store.generate_code("email", "user1", "Alice")
            assert store.approve_code("email", code) is not None
            limits = store._load_json(store._rate_limit_path())
            assert limits.get("_failures:email", 0) == 0
...
class TestCodeExpiry:
    def test_expired_codes_cleaned_up(self, tmp_path):
        with patch("gateway.pairing.PAIRING_DIR", tmp_path):
            ...
            pending = store._load_json(store._pending_path("email"))
            for entry_id in pending:
                pending[entry_id]["created_at"] = time.time() - CODE_TTL_SECONDS - 1
            store._save_json(store._pending_path("email"), pending)
...
class TestRevoke:
    def test_revoke_approved_user(self, tmp_path):
        with patch("gateway.pairing.PAIRING_DIR", tmp_path):
            ...
            revoked = store.revoke("email", "user1")
        assert revoked is True
        with patch("gateway.pairing.PAIRING_DIR", tmp_path):
            assert store.is_approved("email", "user1") is False
...
class TestListAndClear:
    def test_list_approved(self, tmp_path):
        with patch("gateway.pairing.PAIRING_DIR", tmp_path):
            ...
            code = store.generate_code("email", "user1", "Alice")
            store.approve_code("email", code)
            approved = store.list_approved("email")
...
class TestProfileScopedStorage:
    ...
    def test_profile_store_merges_split_pairing_layouts(
        ...
        (legacy_dir / "email-approved.json").write_text(
            '{"legacy-user": {"user_name": "Legacy"}}',
            encoding="utf-8",
        )
        (consolidated_dir / "email-approved.json").write_text(
        ...
        assert store.is_approved("email", "legacy-user")
        assert store.is_approved("email", "new-user")
