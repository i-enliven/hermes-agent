...
def _make_multiplex_runner(monkeypatch):
    """Runner with default allowlist email and secondary open-policy email."""
    ...
    runner.adapters = {Platform.EMAIL: default_adapter}
    runner._profile_adapters = {
        "coder": {Platform.EMAIL: secondary_adapter},
    }
...
def test_active_profile_stamp_resolves_primary_adapter(monkeypatch):
    ...
    assert runner._authorization_adapter(Platform.EMAIL, profile="dev") is default_adapter
...
def test_secondary_allowlist_dm_behavior_ignores_unauthorized(monkeypatch):
    ...
    assert runner._get_unauthorized_dm_behavior(Platform.EMAIL) == "ignore"
...
def test_adapter_auth_check_stamps_secondary_profile(monkeypatch):
    ...
    check = runner._make_adapter_auth_check(Platform.EMAIL, profile_name="coder")
