...
def test_session_store_default_db_uses_runtime_hermes_home(tmp_path, monkeypatch):
    ...
    # Re-pin the import-time DEFAULT_DB_PATH constant to THIS test's home.
    # When hermes_state is already imported (shared pytest process, e.g. an
    # earlier test file imported it at collection), the autouse conftest
    # fixture (step 3b) has re-pointed the constant to its own home and
    # _default_db_path() prefers the re-pointed constant over a fresh
    # get_hermes_home() read — so the env-var redirect alone loses. Re-pinning
    # to our own fake_home keeps the assertion deterministic in both import
    # orders; monkeypatch restores the constant afterwards.
