...
def _make_source(platform: Platform = Platform.LOCAL) -> SessionSource:
...
def _make_event(text: str, *, platform: Platform = Platform.LOCAL) -> MessageEvent:
    return MessageEvent(
        text=text,
...
def _make_runner(session_entry: SessionEntry, *, platform: Platform = Platform.LOCAL):
...
async def test_status_command_bypasses_active_session_guard():
    ...
    adapter = _ConcreteAdapter(platform_config, Platform.LOCAL)
