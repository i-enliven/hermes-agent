...
def _make_adapter():
    adapter = _StubAdapter(PlatformConfig(enabled=True, token="t"), Platform.EMAIL)
...
def _make_event(text="hi", chat_id="42"):
    return MessageEvent(
        ...
        source=SessionSource(platform=Platform.EMAIL, chat_id=chat_id, chat_type="dm"),
    )


def _sk(chat_id="42"):
    return build_session_key(
        SessionSource(platform=Platform.EMAIL, chat_id=chat_id, chat_type="dm")
    )
