...
Thread context is handled by platform adapters
...
- Cross-platform: consistent behavior for Email, Relay, Webhook
"""
...
def _dm_source(platform=Platform.EMAIL, chat_id="D123", thread_id=None, user_id="U1"):
...
def _group_source(platform=Platform.EMAIL, chat_id="C456", thread_id=None, user_id="U1"):
    return SessionSource(
        platform=platform,
        chat_id=chat_id,
        chat_type="group",
...
class TestDMThreadIsolationCrossPlatform:
    ...
    @pytest.mark.parametrize("platform", [Platform.EMAIL, Platform.RELAY, Platform.WEBHOOK])
