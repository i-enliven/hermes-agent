...
class TestAgentCacheBoundedGrowth:
    ...
    def test_is_session_finalizable_real_predicate(self, tmp_path):
        ...
        # Give Email a 'none' policy via the per-platform override; leave the
        # default policy finite ('both') for the Relay case.
        config.default_reset_policy = SessionResetPolicy(mode="both")
        config.reset_by_platform[Platform.EMAIL] = SessionResetPolicy(mode="none")
        ...
        assert store.is_session_finalizable(_entry_for(Platform.EMAIL)) is False
        # default 'both' → finite, will eventually expire.
        assert store.is_session_finalizable(_entry_for(Platform.RELAY)) is True
