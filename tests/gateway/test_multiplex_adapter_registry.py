...
class TestCredentialFingerprint:
    ...
    def test_reads_config_token(self):
        """Adapters store token on `config`, not on self.

        Without the config-token fallback, every adapter in a
...
class TestSecondaryProfileFatalRecovery:
    ...
    async def test_retryable_secondary_fatal_reconnects_with_its_profile_scope(
        ...
        runner._profile_adapters["reviewer"] = {Platform.EMAIL: stale}
        ...
        async def connect(adapter, platform, *, is_reconnect=False):
            ...
            assert platform is Platform.EMAIL
        ...
        await runner._handle_profile_adapter_fatal_error(
            "reviewer", Platform.EMAIL, stale
        )

        assert stale.disconnected is True
        assert Platform.EMAIL not in runner._profile_adapters["reviewer"]
        tasks = list(runner._background_tasks)
        assert len(tasks) == 1
        await tasks[0]
        assert runner._profile_adapters["reviewer"][Platform.EMAIL] is replacement
        assert scoped_homes
    ...
    async def test_secondary_reconnect_does_not_publish_after_shutdown(
        ...
        task = asyncio.create_task(
            runner._run_secondary_profile_reconnect("reviewer", Platform.EMAIL)
        )
        runner._profile_failed_platforms["reviewer"][Platform.EMAIL] = task
...
class TestSecondaryProfileConfigHandling:
    ...
    async def test_secondary_reports_all_port_binding_platforms(self, monkeypatch):
        ...
        reviewer_cfg.platforms = {
            # api_server is a retained port-binding platform (always binds in
            # this fork — no mode-conditional platforms remain). webhook is
            # the second retained port-binding platform; email is not and
            # must not be reported.
            Platform.API_SERVER: PlatformConfig(
                enabled=True, extra={"key": "x" * 16}
            ),
            Platform.WEBHOOK: PlatformConfig(enabled=True, extra={"port": 8644}),
            Platform.EMAIL: PlatformConfig(enabled=True, token="t"),
        }
        monkeypatch.setattr(
    ...
    @pytest.mark.asyncio
    async def test_secondary_distinct_sidecar_credentials_distinct_ports_connect(
        self, monkeypatch
    ):
        """Multiplexing remains supported when sidecar platforms cannot collide."""
        from gateway.config import GatewayConfig, Platform, PlatformConfig
        ...
        teams_p = Platform("teams")
        reviewer_cfg = GatewayConfig(multiplex_profiles=True)
        reviewer_cfg.platforms = {teams_p: PlatformConfig(enabled=True)}
        primary = _SidecarAdapter("primary-secret", 8789)
        secondary = _SidecarAdapter("different-secret", 8790)
        claimed = {
            GatewayRunner._adapter_listener_claim(teams_p, primary): "default"
        }
        ...
        assert runner._profile_adapters["reviewer"][teams_p] is secondary

    @pytest.mark.asyncio
    async def test_failed_sidecar_connect_releases_listener_for_later_profile(
        self, monkeypatch
    ):
        ...
        profile_cfg.platforms = {teams_p: PlatformConfig(enabled=True)}
        failed = _SidecarAdapter("failed-secret", False)
        later = _SidecarAdapter("later-secret", True)
        ...
        assert second == 1
        assert runner._profile_adapters["later"][teams_p] is later


class TestPortBindingConditional:
    """Websocket mode does NOT bind a port; only webhook mode does."""

    @pytest.mark.asyncio
    async def test_websocket_mode_not_rejected(self, monkeypatch):
        """A non-port-binding platform in websocket mode should NOT raise MultiplexConfigError."""
        from gateway.run import MultiplexConfigError
        from gateway.config import GatewayConfig, Platform, PlatformConfig
