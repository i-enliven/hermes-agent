...
platforms, so the literal scan misses "email" and the injection returns
...
class _RelayAdapter:
    """Stub relay adapter fronting email."""
    ...
    def fronts_platform(self, platform):
        return platform == Platform.EMAIL
...
def _email_async_event():
    return {
        "type": "async_delegation",
        "delegation_id": "deleg_relay_route",
        "session_key": "agent:main:email:dm:alice@example.com:1786298425.877239",
        "platform": "email",
        "chat_type": "dm",
...
async def test_injection_resolves_relay_adapter_for_fronted_platform():
    """A gateway whose only adapter is the relay (fronting email) must
    deliver an email-routed completion through it — not drop it as
    'no gateway route'."""
    adapter = _RelayAdapter()
    runner = _runner_with_relay(adapter)

    result = await runner._inject_watch_notification(
        "[delegation completed]", _email_async_event()
    )
...
async def test_injection_still_none_when_platform_not_fronted():
    ...
    targets."""
    adapter = _RelayAdapter()  # fronts email only
    runner = _runner_with_relay(adapter)
    evt = _email_async_event()
    evt["session_key"] = "agent:main:webhook:dm:123:456"
    evt["platform"] = "webhook"
