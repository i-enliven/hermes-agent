...
Trade-off: main hermes is unsupervised under s6. That exactly matches its behavior under tini (the pre-s6 image). The **new** guarantee s6 adds is full supervision for per-profile gateways under `/run/service/`.
