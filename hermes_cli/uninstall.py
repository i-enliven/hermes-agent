...
def main(argv=None) -> int:
    ...
    Useful on Windows, where ``lite``/``full`` rmtree the venv that contains
    the running ``python.exe`` — and a running .exe is mandatory-locked, so
    doing that from the venv's own interpreter half-fails. Running this module
    from a system Python (``PYTHONPATH=<agentRoot>``) sidesteps that:
    ``import hermes_cli`` resolves from source while the venv is torn down.

    This module imports only stdlib + ``hermes_constants`` +
    ``hermes_cli.colors``, so it runs fine under a bare system Python with no
    site-packages from the venv.
    """
    import argparse
    ...
    parser.add_argument(
        ...
        choices=["lite", "full"],
        required=True,
        help="lite = agent + wrapper, keep data; full = everything",
    )
