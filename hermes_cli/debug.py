...
def collect_share_bundle(
    ...
    This is the single source of collection + redaction shared by all
    destinations.  Centralising it guarantees every bundle is built from the
    *same* force-redacted snapshots as the public paste path —
    redaction is the safety boundary, so no path ever sees raw logs.
...
def build_debug_share(
    *,
    ...
    # shared collector so every path builds identical, identically-redacted
    # bundles. The dump header + redaction banner are applied inside
