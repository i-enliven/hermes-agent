...
_KNOWN_DELIVERY_PLATFORMS = frozenset({
    "email", "webhook", "api_server", "msgraph_webhook", "teams", "relay",
})
    ...
    try:
        ...
        if session_store is not None:
            ...
            if platform_enum is not None:
                # Thread destinations key on the parent chat_id for thread
                # messages, so seed with the parent chat_id.
...
def _deliver_result(job: dict, content: str, adapters=None, loop=None) -> Optional[str]:
    ...
    for target in targets:
        ...
        if live_adapter_ready:
            ...
            # Route thread_id via route_metadata (not the DeliveryTarget) so
            # the DeliveryRouter's private-chat topic detection (which demands
            # a reply anchor) is bypassed for anchorless cron sends.
